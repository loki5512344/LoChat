...
/**
 * Менеджер чата - использует MentionService для обработки упоминаний
 */
public class ChatManager {
    ...
    public ChatManager(LoChat plugin) {
        this.plugin = plugin;
        this.mentionService = plugin.getServiceRegistry().get(MentionService.class);
    }
    ...
    public void sendGlobalMessage(Player sender, Component message) {
        ...
        String processedMessage = mentionService.processMentions(messageText, mentionedPlayers);
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            String personalizedMsg = mentionService.getPersonalizedMessage(processedMessage, player);
            ...
            mentionService.notifyIfMentioned(messageText, player, sender);
        }

        // Уведомление упомянутых через @
        mentionService.notifyMentioned(mentionedPlayers);
