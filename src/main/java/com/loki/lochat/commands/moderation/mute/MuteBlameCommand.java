...
public class MuteBlameCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!sender.hasPermission("lochat.muteblame")) { sender.sendMessage(plugin.getMessageConfig().getComponent("errors.no-permission")); return true; }
        if (args.length < 1) { sender.sendMessage("&#CF6679Использование: /lmuteblame <модератор>"); return true; }

        List<MuteData.MuteHistoryEntry> mutes = muteService.getMutesByOperator(args[0]);
        if (mutes.isEmpty()) { sender.sendMessage("&#9878C9Модератор &#7858E9" + args[0] + " &#9878C9не выдавал мутов"); return true; }
        ...
        for (MuteData.MuteHistoryEntry e : mutes) {
            if (idx > 15) { sender.sendMessage("&#B798A8...и ещё " + (mutes.size() - 15) + " записей"); break; }
