...
public class DiscordCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "reload" -> {
                try { discord.reload(); sender.sendMessage(ChatFormatter.parse("&#9878C9Discord &#7858E9перезагружен")); }
                catch (Exception e) { sender.sendMessage(ChatFormatter.parse("&#CF6679Ошибка: " + e.getMessage())); }
            }
            case "test" -> {
                if (!discord.isEnabled()) { sender.sendMessage(ChatFormatter.parse("&#CF6679Discord отключён")); return true; }
                ...
                java.util.concurrent.CompletableFuture.runAsync(() -> {
                    ...
                    else sender.sendMessage(ChatFormatter.parse("&#9878C9Отправлено &#7858E9✓"));
                });
            }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (!sender.hasPermission("lochat.discord.admin")) return null;
        if (args.length == 1) return List.of("status", "reload", "test");
        return null;
    }
}
